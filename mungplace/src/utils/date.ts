...
function getMonthYearDetails(initialDate: Date) {
  ...
  const lastDateString = String(new Date(initialDate.getFullYear(), initialDate.getMonth() + 1, 0).getDate())
...
function getDateWithSeparator(dateString: Date | string, separator: string = '') {
  ...
  return [String(year), String(month).padStart(2, '0'), String(day).padStart(2, '0')].join(separator)
}

// 현재 월과 연도를 기준으로 이전 또는 다음 달의 정보를 가져오는 함수
function getNewMonthYear(prevData: MonthYear, increment: number) {
  const newMonthYear = new Date(prevData.startDate.setMonth(prevData.startDate.getMonth() + increment))
...
export {getDateDetails, getNewMonthYear, getMonthYearDetails, isSameAsCurrentDate, getDateWithSeparator}

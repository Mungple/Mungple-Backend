...
const getJwtToken = async () => {
  try {
    const response = await axios.post(`https://j11e106.p.ssafy.io/api/manager/login?username=manager`)
      ...
      try {
        ...
        const socket = new Client({
          ...
          onConnect: () => {
            ...
            // 산책 기록 위치 수집
            socket.subscribe(`/user/sub/exploration/${explorationId}`, message => {
              try {

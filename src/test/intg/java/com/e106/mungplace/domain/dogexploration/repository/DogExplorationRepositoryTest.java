...
public class DogExplorationRepositoryTest {
    ...
    void Find_DogExploration_From_ExplorationId() {
        ...
        DogExploration dogExploration1 = DogExploration.builder().dog(dog1).exploration(exploration).isEnded(false).build();
        DogExploration dogExploration2 = DogExploration.builder().dog(dog2).exploration(exploration).isEnded(false).build();
        dogExplorationRepository.save(dogExploration1);
        dogExplorationRepository.save(dogExploration2);

        // when
        List<DogExploration> dogExplorations = dogExplorationRepository.findDogExplorationsByExplorationId(exploration.getId());
    ...
    private void createDogs() {
        dog1 = Dog.builder().user(user).dogName("멍멍이1").birth(new Date()).gender(Gender.MALE).weight(30).isDefault(true).build();
        dog2 = Dog.builder().user(user).dogName("멍멍이2").birth(new Date()).gender(Gender.FEMALE).weight(25).isDefault(false).build();

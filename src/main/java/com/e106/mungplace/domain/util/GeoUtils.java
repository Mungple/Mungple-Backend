...
public interface GeoUtils {
    ...
    static Point calculateNorthWestPoint(Point point, int sideLength) {
        GlobalCoordinates nw = calculateOffset(new GlobalCoordinates(point.lat(), point.lon()), -45, sideLength / Math.sqrt(2));

        return new Point(nw.getLatitude(), nw.getLongitude());
    }

    static Point calculateSouthEastPoint(Point point, int sideLength) {
        GlobalCoordinates nw = calculateOffset(new GlobalCoordinates(point.lat(), point.lon()), 135, sideLength / Math.sqrt(2));

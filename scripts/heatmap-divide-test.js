...
function sendMessage(socket, userId) {
    ...
    const message = JSON.stringify({
        ...
        side: 500,
        point: {
            lat: 35.08444655299801,
            lon: 128.8775216840485,
        },
    });
    ...
    requestIterMaps.set(requestId, 0);
...
function handleMessage(msg) {
    ...
    requestIterMaps.set(requestId, requestIterMaps.get(requestId) + 1);
    ...
    if (requestTimestamps.has(requestId)) {
        ...
        if(requestIterMaps.get(requestId) === 1) {
            messageLatency.add(latency);
        }

        if(requestIterMaps.get(requestId) === DIVIDE ** ITER) {
